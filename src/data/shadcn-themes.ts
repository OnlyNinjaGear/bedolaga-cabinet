...
export const BASE_COLORS = THEMES.filter((t) => BASE_COLOR_NAMES.includes(t.name));
export const THEME_COLORS = THEMES.filter((t) => THEME_COLOR_NAMES.includes(t.name));
...
/**
 * Combines a base color theme (neutrals) with a theme color (primary accent).
 * Base provides: background, foreground, card, popover, secondary, muted, accent,
 * border, input, ring, chart-*, sidebar-*, destructive.
 * Theme provides: primary, primary-foreground (and overrides chart-*, sidebar-primary-*).
 */
// Keys taken from the accent theme color (everything else comes from base neutral)
const ACCENT_KEYS = [
  'primary',
  'primary-foreground',
  'ring',
...
export function combineTheme(
  ...
  const combine = (mode: 'light' | 'dark') => {
    // Start from base neutral palette (background, card, muted, border, etc.)
    const result = { ...baseColor.cssVars[mode] };
    const theme = themeColor.cssVars[mode];

    // Only overlay accent-specific keys from the theme color
 ...
 */
export function applyTheme(vars: Record<string, string>, _isDark: boolean) {
  const root = document.documentElement;

  // Apply on :root — the vars themselves are mode-specific
