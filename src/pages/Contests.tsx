  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <GamepadIcon />
        <h1 className="text-foreground text-2xl font-bold sm:text-3xl">{t('contests.title')}</h1>
      </div>

      {/* Game Modal */}
      <Dialog open={selectedContest !== null} onOpenChange={(open) => !open && handleCloseGame()}>
        <DialogContent className="max-h-[80vh] max-w-lg overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{selectedContest?.name}</DialogTitle>
          </DialogHeader>
          ...
          {gameData && !result && (
            <div className="space-y-4">
              <p className="text-muted-foreground">{gameData.instructions}</p>
              ...
              {['cipher', 'emoji', 'anagram'].includes(gameData.game_type) && (
                <form
                  ...
                  className="space-y-3"
                >
                  <div className="bg-muted rounded-lg p-4 text-center font-mono text-2xl">
                  ...
                  <input
                    ...
                    className="border-border bg-muted focus:border-primary w-full rounded-lg border px-4 py-3 focus:outline-none"
                  />
                  <Button
                    type="submit"
                    disabled={submitAnswerMutation.isPending}
          ...
          {result && (
            <Button variant="secondary" onClick={handleCloseGame} className="mt-4 w-full">
              {t('common.close')}
            </Button>
          )}
        </DialogContent>
      ...
      {contests && contests.length > 0 ? (
            ...
            <Card key={contest.id}>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {contest.description && (
                    <p className="text-muted-foreground mt-1 text-sm">{contest.description}</p>
                  )}
                </div>
              ...
              <div className="mt-4">
                {contest.already_played ? (
                  ...
                  <Button onClick={() => handlePlayContest(contest)} className="w-full">
                    {t('contests.play')}
                  </Button>
                )}
              </div>
        ...
        <Card className="py-12 text-center">
          <GamepadIcon />
          <p className="text-muted-foreground mt-4">{t('contests.noContests')}</p>
        </Card>
      )}
    </div>
  );
